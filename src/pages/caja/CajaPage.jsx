...
export default function CajaPage() {
  ...
  async function fetchCaja() {
    ...
    const [{ data: cerradas }, { data: openData }] = await Promise.all([
      supabase.from('cuentas')
        .select('*, abierta:profiles!cuentas_abierta_por_fkey(nombre), cerrada:profiles!cuentas_cerrada_por_fkey(nombre), cargos(cantidad, precio_unit, subtotal, platillos(nombre, emoji))')
        .eq('estado', 'cerrada')
  ...
  function imprimirCorte() {
    ...
    const filasCerradas = cierres.map(c => {
      const platillosHTML = (c.cargos ?? []).map(cargo => `
        <tr style="background:#FAFAF8">
          <td style="padding:4px 4px 4px 24px;color:#7A6E5F;font-size:11px">
            ${cargo.platillos?.emoji ?? ''} ${cargo.platillos?.nombre ?? ''}
          </td>
          <td style="padding:4px;color:#7A6E5F;font-size:11px">×${cargo.cantidad}</td>
          <td style="text-align:right;padding:4px;color:#7A6E5F;font-size:11px">$${Number(cargo.subtotal).toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
          <td></td>
        </tr>`).join('')
        ...
        <tr>
          <td colspan="4" style="padding-top:10px">
            ...
            <span style="font-size:10px;color:#7A6E5F">
              ...
              <span style="text-transform:capitalize">${(c.forma_pago ?? '').replace('_', ' ')}</span>
            </span>
            ${c.nota_cierre ? `<br><span style="font-size:10px;color:#C17B2A">${c.nota_cierre}</span>` : ''}
          </td>
        </tr>
        ${platillosHTML}
        <tr style="border-top:0.5px solid #E8DCC8">
          <td colspan="3" style="text-align:right;font-weight:600;padding:4px 4px 10px">Total cobrado</td>
          <td style="text-align:right;font-weight:700;padding:4px 4px 10px">$${Number(c.total_cobrado).toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
        </tr>`
    }).join('')
...
<html lang="es"><head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    body{font-family:'Helvetica Neue',Arial,sans-serif;font-size:12px;color:#2C2416;padding:24px;max-width:640px;margin:0 auto}
    ...
    .st{font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:1.5px;color:#7A6E5F;border-bottom:0.5px solid #E8DCC8;padding-bottom:6px;margin-bottom:10px}
    table{width:100%;border-collapse:collapse}
    td{padding:6px 4px;border-bottom:0.5px solid #F0EDE8;vertical-align:top;line-height:1.5}
  ...
  <div class="section">
    <div class="st">Desglose por forma de pago</div>
    <table>${filasPago || '<tr><td colspan="2" style="color:#7A6E5F">Sin cierres</td></tr>'}</table>
    <div class="total-row"><span style="font-size:11px;font-weight:500;letter-spacing:1px;text-transform:uppercase">Total en caja</span><span style="font-size:22px;font-weight:600">$${resumen.total.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</span></div>
  </div>
  <div class="section">
    <div class="st">Detalle de cuentas cerradas (${cierres.length})</div>
    <table>${filasCerradas || '<tr><td colspan="4" style="color:#7A6E5F">Sin cierres</td></tr>'}</table>
  </div>
  ${esHoy && abiertas.length > 0 ? `<div class="section">
    <div class="st">Cuentas abiertas (${abiertas.length}) — pendientes</div>
    <table>
  ...
  <div class="footer">Sistema Mazul · Generado el ${new Date().toLocaleDateString('es-MX')} a las ${hora}<br>Este documento es de uso interno</div>
</body></html>`
  ...
  return (
    <div className="pb-24">
      <AppHeader
        ...
        action={<button onClick={imprimirCorte} className="text-xs bg-mazul-bark text-mazul-cream px-3 py-1.5 rounded-lg font-medium">🖨️ Corte</button>}
      />

      {/* Selector de fecha */}
      <div className="px-4 py-3 bg-white border-b border-mazul-sand/60 flex items-center gap-3">
        <button onClick={() => { const d = new Date(fecha+'T12:00:00'); d.setDate(d.getDate()-1); setFecha(fechaLocal(d)) }} className="w-8 h-8 rounded-lg bg-mazul-mist text-mazul-stone flex items-center justify-center text-sm">‹</button>
        <input type="date" value={fecha} max={fechaLocal(new Date())} onChange={e => setFecha(e.target.value)} className="flex-1 text-center text-sm font-medium text-mazul-bark bg-transparent border-none outline-none" />
        <button onClick={() => { const d = new Date(fecha+'T12:00:00'); d.setDate(d.getDate()+1); if(fechaLocal(d)<=fechaLocal(new Date())) setFecha(fechaLocal(d)) }} className={`w-8 h-8 rounded-lg bg-mazul-mist flex items-center justify-center text-sm ${esHoy?'opacity-30':'text-mazul-stone'}`} disabled={esHoy}>›</button>
        {!esHoy && <button onClick={() => setFecha(fechaLocal(new Date()))} className="text-xs text-mazul-moss font-medium">Hoy</button>}
      </div>
          ...
          <div className="space-y-2">
            {cierres.map(c => (
              <div key={c.id} className="card p-3">
                ...
                {(c.cargos??[]).length > 0 && (
                  <div className="border-t border-mazul-sand/40 pt-2 space-y-1">
                    {(c.cargos??[]).map((cargo, i) => (
                      <div key={i} className="flex justify-between items-center">
                        <p className="text-xs text-mazul-stone">{cargo.platillos?.emoji} {cargo.platillos?.nombre} ×{cargo.cantidad}</p>
                        <p className="text-xs text-mazul-stone">${Number(cargo.subtotal).toLocaleString('es-MX',{minimumFractionDigits:0})}</p>
                      </div>
                    ))}
                  </div>
                )}
